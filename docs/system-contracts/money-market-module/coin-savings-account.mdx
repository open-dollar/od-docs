---
title: Coin Savings Account - Detailed Documentation
...
Coin Savings Account allows users to deposit `coin` and activate the Coin Savings Rate and earning savings on their `coin`. The savings rate is set by Open Dollar Governance, and will typically be less than the base stability fee to remain sustainable. The purpose of Coin Savings Account is to offer another incentive for holding Coin.
...
The values of `savingsRate` and `accountingEngine` can be changed by an authorized address in the contract (i.e. Open Dollar Governance). The values of `accumulatedRates`, `savings`, `totalSavings`, and `updateTime` are updated internally in the contract and cannot be changed manually.
