...
### Global Settlement Properties Compared to MCD <a href="#current-implementation-properties-of-shutdown" id="current-implementation-properties-of-shutdown" />
...
### Settlement Stages <a href="#the-shutdown-mechanism-9-crucial-steps" id="the-shutdown-mechanism-9-crucial-steps" />

#### 1. `shutdownSystem()` : <a href="#1-cage" id="1-cage" />

* Freezes user entry-points
* Cancels collateral/surplus auctions
* Starts the cooldown period

#### 2. `freezeCollateralType(collateralType)` : <a href="#2-cage-collateralType" id="2-cage-collateralType" />
...
#### 3. `processSAFE(collateralType, safe)` <a href="#3-skim-collateralType-urn" id="3-skim-collateralType-urn" />
...
#### 4. `shutdownCooldown` or `fastTrackAuction` <a href="#4-wait-or-skip" id="4-wait-or-skip" />
...
#### 5. `freeCollateral(collateralType)` : <a href="#5-free-collateralType" id="5-free-collateralType" />
...
#### 6. `setOutstandingCoinSupply()` <a href="#6-thaw" id="6-thaw" />
...
#### 7. `calculateCashPrice(collateralType)` <a href="#7-flow-collateralType" id="7-flow-collateralType" />
...
#### 8. `prepareCoinsForRedeeming(coinAmount)` <a href="#8-pack-wad" id="8-pack-wad" />

Put some coins into a `coinBag` in order to `redeemCollateral`. The bigger the bag, the more collateral the user can claim.

#### 9. `redeemCollateral(collateralType, collateralAmount)` <a href="#9-cash-collateralType-wad" id="9-cash-collateralType-wad" />
...
* SAFE (processSAFE/GlobalSettlement) Keeper - is a tool to process underwater SAFEs if not all undercollateralized SAFES are accounted for. This Keeper could be used by GEB Stakeholders such as large Coin holders/custodians, ODG governors, Redemption keepers and more.
