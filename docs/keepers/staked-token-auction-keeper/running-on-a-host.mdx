...
A basic debt staked auction bidding model can be found in `models/debt_staked_model.py`. This model retrieves the latest ODG/USD price from Coingecko and will automatically place bids in an auction.

You probably want to modify the following variables in `models/debt_staked_model.py`:

* `MAXIMUM_ODG_MULTIPLIER`: the maximum acceptable ODG price to use when bidding. Default: `0.90` meaning the maximum price to pay when biding for ODG (with OD) is 90% of the current ODG/USD market price coming from Coingecko
* `MINIMUM_ODG_MULTIPLIER`: the minimum ODG price to use when bidding. This will determine your opening bid. Default: `0.50` meaning the minimum price to pay when biding for ODG (with OD) is 50% of the current ODG/USD market price from Coingecko
